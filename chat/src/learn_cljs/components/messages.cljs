(ns learn-cljs.components.messages
  (:require
...
(defn message-state-accessor
  ...
  (let [sender (state/person-by-username app (:sender message))
    ...
    (assoc message
           :author {:name name :initial initial}
...
(defn accessor
  ...
  (->> app
       :messages
       (map #(message-state-accessor app %))))
...
(defn render-message
  ...
  (dom/article
    ...
    (dom/div "message-content" (dom/p nil (:content message)))))

(defn render
  [el messages]
  (dom/with-children el (map render-message messages)))
