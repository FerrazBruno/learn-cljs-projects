...
(defn replace-contact
  [contact-list idx input]
  (assoc contact-list
         idx (make-contact input)))
...
(defn render-contact-list-item
  [idx contact selected?]
...
(defn form-field
  ([id value label]
   (form-field id value label "text"))
  ([id value label type]
   [:div {:class "field"}
       ...
       [:div {:class "hero is-fullheight"}
        (if (:editing? state)
          (render-contact-details
            (get-in state [:contacts (:selected state)] {}))
          no-contact-details)]]]])))
